...
description: |
  ...
  Auto-activates in projects with `cacheComponents: true` in next.config.
---
...
Use proactively when working in Next.js projects with `cacheComponents: true` in next.config.
Covers: `'use cache'` directive, `cacheLife()`, `cacheTag()`, `updateTag()`/`revalidateTag()`, static vs dynamic boundaries, cache debugging.
