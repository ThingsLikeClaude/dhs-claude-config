...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-frontend` | 프론트엔드 컨벤션 (네이밍, use client, DataGrid, 에러 핸들링) | `frontend/src/**/*.{tsx,ts,css}` |
| `verify-domain` | DDD 도메인 레이어 (순수 함수, 상태 전이, Thin Controller) | `frontend/src/domain/**`, `frontend/src/app/actions/**` |
| `verify-db` | DB 레이어 (마이그레이션, RPC, RLS, 타입 동기화) | `supabase/migrations/**`, `frontend/scripts/sql/**`, `frontend/src/services/supabase/**`, `frontend/src/types/database*` |
...
3. **생성** — `.claude/skills/verify-<name>/SKILL.md`를 템플릿에 따라 생성합니다
