...
**IMPORTANT (Windows compatibility):** On Windows, bash `/tmp` and Python `Path('/tmp')` resolve to different directories (Git Bash maps `/tmp` to `$LOCALAPPDATA/Temp`, Python maps it to `\tmp` on the current drive). This script uses `~/.notebooklm/` for all temp files to avoid this mismatch.

```bash
cat > ~/.notebooklm/nlm_login.py << 'PYEOF'
import json, os, time
...
python3 ~/.notebooklm/nlm_login.py > ~/.notebooklm/login_output.txt 2>&1 &
...
Once the user confirms they are on the NotebookLM homepage, save the session via Python (to ensure path consistency):

```bash
python3 -c "from pathlib import Path; (Path.home() / '.notebooklm' / 'save_signal').touch()"
sleep 8
cat ~/.notebooklm/login_output.txt
```
